...
public class GUI extends JFrame implements ActionListener {
  ...
  public int[] timePerFrame = new int[15];					//enth�lt jede zeit jedes frames, das ins GIF kommt
  public int[] indexSammler = new int[15];    				//enth�lt jeden index, der ins GIF kommt
...
final JList list3 = new JList(model3); 
  ...
  final JSlider slider = new JSlider(JSlider.HORIZONTAL,42,10000,1000);		//geht von 42-10000 und hat den standartwert 1000  ,  global um slider.getValue() machen zu k�nnen
  ...
  private void initialize() {
    ...
    final JPanel panel = new JPanel();        //Gro�es Panel, auf dem alle anderen Panels und Buttons etc. aufbauen
    JPanel buttonPanel = new JPanel();        //nur f�r die 8x8 buttons
    frame.getContentPane().add(panel);		  //Panel zum Frame hinzugef�gt
    panel.setLayout(null);					  //Kein Layout --> Direkte Koordinatenangabe der Items ben�tigt
        
    
    buttonPanel.setBounds(0, 0, 600, 600);	 				 //Groesse des Panels f�r die Buttons festlegen
    buttonPanel.setLayout(new GridLayout(8, 8, -1, -1));	 //Als Layout natuerlich das GridLayout, welches ein 8x8 "Schachfeld" macht. Zwischen den Quadraten ist die L�cke -1, also keine L�cke
    ...
    for (int i = 0; i < buttons.length; i++) { //alle 64 Buttons entwerfen und auf das Panel setzen   	
      ...
      button.addActionListener(this);          //Action Listener zu jedem Button hinzuf�gen 
      buttons[i] = button;
...
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX    Menubar
    ...
    frame.setJMenuBar(menuBar);										//Menubar zum Frame hinzugef�gt
    ...
    menuBar.add(file);      										//Menu zur Menubar hinzugef�gt
    menuBar.add(Edit);       										
    ...
    OpenFile.setIcon(openFile);										//Image zum JMenuItem zugef�gt
...
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX		Buttons
    ...
    slider.setMajorTickSpacing(1000);		//JSlider f�r die zeitbestimmung
...
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX		PopupMenu 
    ...
    final JPopupMenu popupMenu = new JPopupMenu();   	//Popupmenu f�r Liste 1        
    ...
    final JPopupMenu popupMenu2 = new JPopupMenu();    //Popupmenu f�r Liste 2       
    ...
    final JPopupMenu popupMenu3 = new JPopupMenu();   //Popupmenu f�r Liste 3         
    ...
    save.addActionListener(this);			//action listener f�r alle popupMenu items hinzugef�gt
    ...
    int framedisplaytime=0; 
      ...
      deselectButton.addActionListener(new ActionListener() {
          public void actionPerformed(ActionEvent e) {        	  	        	  	
        	  ...
        	  frameZaehler=0; 									//sonst z�hlt es zu hoch und man kann keine GIFs mehr sehen
      ...
      for (int i = 0; i <= 14; i++) {
    	  indexInVideo[i]=false;		//kein frame geh�rt bisher in das n�chste video rein also ist das gesamte Array auf "false"
	  }
        	  ...
        	  model3.addElement("GIF " + zaehler);			//In der dritten Liste wird ein Item hinzugefuegt
          	  ...
          	  for (int i = 0; i <= 14; i++) {
          			...
          			if (model2.getElementAt(i) != null) {		//geht jedes tabellenitem durch und guckt ob es einen inhalt hat, wenn ja ....
          				...
          				indexZaehler++;
    					indexSammler[indexZaehler]=i+1;			//enth�lt alle index die ins video kommen m�ssen
					}         			
				}
      ...
      list.addListSelectionListener(new ListSelectionListener() {         //wenn ein item in liste 1 angeklickt wird
            ...
            public void valueChanged(ListSelectionEvent arg0) {
                if (!arg0.getValueIsAdjusting()) { 						  //wenn der r�ckgabewert des events =true ist
      ...
      list.addMouseListener(new MouseAdapter() {								//frame l�sst sich mit doppelklick in liste 2 verschieben
          @SuppressWarnings("unchecked")
      ...
      list2.addMouseListener(new MouseAdapter() {								//frame l�sst sich per doppelklick l�schen in liste 2
          public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() == 2) {
      ...
      list3.addMouseListener(new MouseAdapter() {								//frame l�sst sich per doppelklick "renamen"   in liste 3
          @SuppressWarnings("unchecked")
		public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() == 2) {
      ...
      list.addMouseListener(new MouseAdapter() {
          ...
          if (SwingUtilities.isRightMouseButton(me)			//wenn rechte maustaste gedr�ckt wurde
          && !list.isSelectionEmpty()						//UND wenn NICHT nichts angeklickt ist
          && list.locationToIndex(me.getPoint())			//UND der ursprung des klicks 
          == list.getSelectedIndex()) {						//dem ausgew�hlten item enspricht
      ...
      list2.addMouseListener(new MouseAdapter() {
          ...
          == list2.getSelectedIndex()) {					//dem ausgew�hlten item enspricht
      ...
      list3.addMouseListener(new MouseAdapter() {
          ...
          == list3.getSelectedIndex()) {					//dem ausgew�hlten item enspricht
      ...
      rename.addActionListener(new ActionListener() {					//action listener f�r popupmenu item "rename" in liste 1
          @SuppressWarnings("unchecked")
		public void actionPerformed(ActionEvent e) {
      ...
      rename2.addActionListener(new ActionListener() {					//action listener f�r popupmenu item "rename" in liste 2
          @SuppressWarnings("unchecked")
		public void actionPerformed(ActionEvent e) {
        	  ListSelectionModel selmodel = list2.getSelectionModel();
        	  int index = selmodel.getMinSelectionIndex();
      ...
      rename3.addActionListener(new ActionListener() {					//action listener f�r popupmenu item "rename" in liste 3
          @SuppressWarnings("unchecked")
		public void actionPerformed(ActionEvent e) {
        	  ListSelectionModel selmodel = list3.getSelectionModel();
        	  int index = selmodel.getMinSelectionIndex();
      ...
      delete.addActionListener(new ActionListener() {					//action listener f�r popupmenu item "delete" in liste 1
      ...
      delete2.addActionListener(new ActionListener() {					//action listener f�r popupmenu item "delete" in liste 2
          public void actionPerformed(ActionEvent event) {
        	  ListSelectionModel selmodel = list2.getSelectionModel();
        	  int index = selmodel.getMinSelectionIndex();
        	  if (index >= 0)
      ...
      delete3.addActionListener(new ActionListener() {					//action listener f�r popupmenu item "delete" in liste 3
          public void actionPerformed(ActionEvent event) {
        	  ListSelectionModel selmodel = list3.getSelectionModel();
        	  int index = selmodel.getMinSelectionIndex();
        	  if (index >= 0)
